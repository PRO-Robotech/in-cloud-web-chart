...
- type: delete
  props:
    ...
    # Target resource identity
    # =========================
    name: {{ .name | default "{reqsJsonPath[0]['.items.0.metadata.name']['-']}" | quote }}
...
{{- $itemsPath := ternary .itemsPath ".items.0" (hasKey . "itemsPath") -}}
- type: cordon
  props:
    # =========================
    # API endpoint (parameterized)
...
{{- $pathToValue := .pathToValue | default "/spec/paused" -}}
{{- $jsonPath := replace "/" "." $pathToValue -}}
{{- $itemsPath := ternary .itemsPath ".items.0" (hasKey . "itemsPath") -}}
- type: suspend
  props:
    # =========================
    ...
    visibleWhen:
      criteria: notEquals
      value: {{ printf "{reqsJsonPath[0]['%s%s']['-']}" $itemsPath $jsonPath | quote }}
      valueToCompare: "true"
{{- end -}}
...
- type: scale
  props:
    ...
    # Scale configuration
    # =========================
    currentReplicas: {{ .currentReplicas | default "{reqsJsonPath[0]['.items.0.spec.replicas']['-']}" | quote }}
    name: {{ .name | default "{reqsJsonPath[0]['.items.0.metadata.name']['-']}" | quote }}
    namespace: {{ .namespace | default "{3}" | quote }}
